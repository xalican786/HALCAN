...
// Flash capacity, propeller, cycles, revenue, chain status
...
import {
  H, PORT, SYSTEM, VERSION, EXECUTOR,
  TOTAL_FLASH, BALANCER_FLASH, AAVE_FLASH,
  PER_CYCLE_TARGET, CYCLES_PER_DAY,
  PROPELLER, ACTIVE_PROPELLER, SAB_SIZE,
  CONTRACT,
} from './config.js'
...
function fullState() {
  ...
  const progress = getProgress(H2)
  const velocity = getVelocity(H2)
  ...
  return {
    ...
    mb:           H2[H.MB]         | 0,
    executor:     EXECUTOR,
    halcanAddr:   CONTRACT.HALCAN  || 'deploying...',
    version:      VERSION,
...
export function startDashboard(SAB) {
  SAB_REF = SAB
  srv.listen(PORT, () => {
    console.log(`[DASHBOARD] Halcan :${PORT} | Flash $70B | $7B/cycle | /ping`)
  })
}
